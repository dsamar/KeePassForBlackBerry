...
enum CryptAlgorithm {
	Rijndael_Cipher = 0, Twofish_Cipher = 1
};
...
public:
	...
	KpxDateTime(const QDate & date) :
			QDateTime(date) {
	};
	KpxDateTime(const QDate & date, const QTime & time, Qt::TimeSpec spec =
			Qt::LocalTime) :
			QDateTime(date, time, spec) {
	};
	KpxDateTime(const QDateTime & other) :
			QDateTime(other) {
	};
	...
	static KpxDateTime fromString(const QString& string, Qt::DateFormat format =
			Qt::TextDate);
...
public:
	 ...
	 */
	virtual IEntryHandle* addEntry(const CEntry* NewEntry,
			IGroupHandle* Group)=0;

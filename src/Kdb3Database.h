...
class Kdb3Database: public QObject, public IDatabase, public IKdbSettings {
Q_OBJECT
public:
	class StdGroup;
	class StdEntry;
	...
	virtual QList<IEntryHandle*> search(IGroupHandle* Group,
			const QString& SearchString, bool CaseSensitve, bool RegExp,
			bool Recursive, bool* Fields);
	virtual QFile* file() {
		return File;
	}
	virtual bool changeFile(const QString& filename);
	virtual void setCryptAlgorithm(CryptAlgorithm algo) {
		Algorithm = algo;
	}
	virtual CryptAlgorithm cryptAlgorithm() {
		return Algorithm;
	}
	virtual unsigned int keyTransfRounds() {
		return KeyTransfRounds;
	}
	virtual void setKeyTransfRounds(unsigned int rounds) {
		KeyTransfRounds = rounds;
	}
	...
	virtual bool setCompositeKey(const QString& password,
	...
	virtual void moveGroup(IGroupHandle* Group, IGroupHandle* NewParent,
			int Position);
	...
	inline bool hasPasswordEncodingChanged() {
		return passwordEncodingChanged;
	}
	;
...
private:
	...
	bool readEntryField(StdEntry* entry, quint16 FieldType, quint32 FieldSize,
			quint8 *pData);
	bool readGroupField(StdGroup* group, QList<quint32>& Levels,
			quint16 FieldType, quint8 *pData);
	...
	void serializeEntries(QList<StdEntry>& EntryList, char* buffer,
			unsigned int& pos);
	...
	bool searchStringContains(const QString& search, const QString& string,
			bool Cs, bool RegExp);
	void getEntriesRecursive(IGroupHandle* Group,
			QList<IEntryHandle*>& EntryList);
	...
	static bool EntryHandleLessThan(const IEntryHandle* This,
			const IEntryHandle* Other);
	static bool EntryHandleLessThanStd(const IEntryHandle* This,
			const IEntryHandle* Other);
	static bool StdEntryLessThan(const Kdb3Database::StdEntry& This,
			const Kdb3Database::StdEntry& Other);
...
public:
	static void transform(quint8* src, quint8* dst, quint8* KeySeed,
			int rounds);
